<template>
    <div class="my-order-container" :class="filterBills.length > 0 ? '' : 'fit-screen'">
        <div v-if="filterBills.length > 0" class="my-order-cards">
            <div v-for="b in filterBills.slice().reverse()" class="card" :key="b.bill_id">
                ...
                <div class="d-flex flex-wrap flex-sm-nowrap justify-content-between card-summary">
                    ...
                    <div class="w-100 text-center py-1 px-2"><span>Thời gian đặt:</span> {{ b.bill_when }}</div>
                </div>
                <div class="d-flex flex-wrap flex-sm-nowrap justify-content-between card-summary">

                    <div class="w-100 text-center py-1 px-2"><span>Tổng:</span> {{ b.bill_total }}đ</div>
                    <div class="w-100 text-center py-1 px-2"><span>Địa chỉ:</span>{{ " " + b.bill_address }}
                    </div>
                    <div class="w-100 text-center py-1 px-2"><span>SĐT:</span>{{ " " + b.bill_phone }}
                    </div>
                </div>
        ...
        <div v-else class="box-content row no-food">
            ...
            <router-link class="btn" to="/menu">Đặt món ngay</router-link>
        </div>
...
export default {
    ...
    data() {
        return {
            avaiableStatus: ["Huỷ", "Xác nhận", "Đang chuẩn bị", "Kiểm tra", "Vận chuyển", "Đã giao"],
            allBills: [],
